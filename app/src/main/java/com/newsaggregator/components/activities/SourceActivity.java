package com.newsaggregator.components.activities;
...
import com.newsaggregator.components.fragments.SourcesFragment;
import com.newsaggregator.data.parsers.HttpsXmlParser;
import com.newsaggregator.data.sqlite.DAOService;
import com.newsaggregator.data.sqlite.entities.Source;
...
public class SourceActivity extends BaseActivity implements SourcesFragment.Listener {
    ...
    public void deleteSource(long sourceId) {
        daoService.deleteSource((int) sourceId);
        deleteChannelFromList(sourceId);
        replaceSourceFragment();
        showToast(R.string.activity_source_channel_deleted);
    }

    private void deleteChannelFromList(long sourceId) {
        int indexForDelete = -10;
        for (int i = 0; i < channels.size(); i++) {
            if (channels.get(i).getDbSourceId() == sourceId) {
                indexForDelete = i;
                break;
            }
        }
        if (indexForDelete != -10) {
            channels.remove(indexForDelete);
        }
    }
